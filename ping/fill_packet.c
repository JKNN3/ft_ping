...
void    fill_ip_header(struct ip *packet, t_conf *conf){
    ...
    packet->ip_off = 0;//    htons(SET_THE_DONT_FRAGMENT_THE_PACKET_BIT_TO_TRUE);
...
void    fill_icmp_msg(struct icmp *packet, t_conf *conf){
    ...
    packet->icmp_cksum = 0;
    packet->icmp_id = 2; // mike muss did : in = getpid() & 0xFFFF pour avoir juste les 16 derniers bits
    ...
    packet->icmp_cksum = compute_checksum((void *)packet, 64);
}

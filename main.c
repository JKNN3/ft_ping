...
void    fill_ip_header(struct ip *ip_header, char dest_str[100]){
    ...
    ip_header->ip_tos = 0;
    ip_header->ip_len = IP_TOTAL_LEN; // htons
    ip_header->ip_id = 0; // ?? 
    ip_header->ip_off = SET_DONT_FRAGMENT_BIT_TO_TRUE ; // htons
    ...
    ip_header->ip_sum = compute_checksum((void*)ip_header, IP_TOTAL_LEN); // -> u_short () 2 bytes
}

void    fill_icmp_message(struct icmp *icmp_message){
    icmp_message->icmp_type = ICMP_ECHO;
    icmp_message->icmp_code = ICMP_ECHOREPLY;
    icmp_message->icd_id = 0; // mike muss did : in = getpid() & 0xFFFF pour avoir juste les 16 derniers bits
                                // car c'est un u_int_16. C'est important poiur l'identification du paquet retour;
    icmp_message->icd_seq= 0; // nombre de paquets transmis, a ++ quand on envoie un paquet, struct principale.
    gettimeoftheday((void *)icmp_message /*+ ICMP_HEADER_LEN*/, NULL); //check ça  
